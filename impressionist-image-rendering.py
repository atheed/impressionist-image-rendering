...
def main_execute_part1(name_of_file):
    '''
    Function to produce output for Part 1
    '''
    ...
    print name_of_file + " part 1 rendering done! \n"
    time.time()
...
def main_execute_part2(name_of_file):
    '''
    Function to produce output for Part 2
    '''
    # Read image and convert it to double, and scale each R,G,B
    # channel to range [0,1].
    imRGB = array(Image.open(name_of_file))
    ...
    while len(np.where(canvas==-1)[1])>0:
        # finding a negative pixel
        # Randomly select stroke center from unfilled pixels
    ...
    print name_of_file + " part 2 rendering done! \n"
    time.time()
...
def main_execute_part3(name_of_file):
    '''
    Function to produce output for Part 3
    '''
    # Read image and convert it to double, and scale each R,G,B
    # channel to range [0,1].
    imRGB = array(Image.open(name_of_file))
    ...
    # make images monochrome with correct intensities
    r, g, b = imRGB[:,:,0], imRGB[:,:,1], imRGB[:,:,2]
    gray = 0.30*r + 0.59*g + 0.11*b
    s = canny(gray, 2.0, 25, 10)
    
    print name_of_file + " part 3 rendering done! \n"
    
    return s
    
    
def main_execute_part4(name_of_file, radius):
    '''
    Function to produce output for Part 4, for given radius
    '''
    # Read image and convert it to double, and scale each R,G,B
    # channel to range [0,1].
    imRGB = array(Image.open(name_of_file))
    ...
    r, g, b = imRGB[:,:,0], imRGB[:,:,1], imRGB[:,:,2]
    monotone = 0.30*r + 0.59*g + 0.11*b
    
    # create Canny edgels map using sigma=2.0 and custom-set thresholds
    canny_map = canny(monotone, 2.0, 25, 10)
    ...
    while len(np.where(canvas==-1)[1])>0:
        ...
        if canny_map[cntr[1], cntr[0]] == 1:
            ...
            for right_distance in range(0, right):
                ...
                # if either of these is found, that is the right endpoint, so
                # right half length = the distance walked
    ...
    print name_of_file + " part 4 rendering, with radius " + str(radius) + " done! \n"
    time.time()
...
def main_execute_part5(name_of_file, radius):
    '''
    Function to produce output for Part 5, for given radius
    '''
    # Read image and convert it to double, and scale each R,G,B
    # channel to range [0,1].
    imRGB = array(Image.open(name_of_file))
    ...
    # make image into monotonem using appropriate intensities
    r, g, b = imRGB[:,:,0], imRGB[:,:,1], imRGB[:,:,2]
    monotone = 0.30*r + 0.59*g + 0.11*b
    
    # create Canny edgels map using sigma=2.0 and custom-set thresholds
    canny_map = canny(monotone, 4.0, 18, 5)
    ...
    thetaArray = gradientCalculator(monotone, 4.0)
    
    time.time()
    time.clock()
    
    p = 0
    while len(np.where(canvas<0)[1])>0:
        # half length of drawn lines, in left and right directions
        left, right = 5, 5
    
        # find all unfilled pixels
        unfilled = np.where(canvas<0)
...
if __name__ == "__main__":
    # call appropriate function and produce output for both images for Part 1
    canvas_1_part1 = main_execute_part1('orchid.jpg')
    colorImSave('orchid_part1_output.png', canvas_1_part1)
    
    canvas_2_part1 = main_execute_part1('RV.jpg')
    colorImSave('RV_part1_output.png', canvas_2_part1)
    
    # call appropriate function and produce output for both images for Part 2
    canvas_1_part2 = main_execute_part2('orchid.jpg')
    colorImSave('orchid_part2_output.png', canvas_1_part2)
    
    canvas_2_part2 = main_execute_part2('RV.jpg')
    colorImSave('RV_part2_output.png', canvas_2_part2)
    
    # call appropriate function and produce output for both images for Part 3
    canvas_1_part3 = main_execute_part3('orchid.jpg')
    colorImSave('orchid_part3_output.png', canvas_1_part3)
    
    canvas_2_part3 = main_execute_part3('RV.jpg')
    colorImSave('RV_part3_output.png', canvas_2_part3)
    
    # call appropriate function and produce output for both images for Part 4,
    # for radius = 3
    canvas_1_part4 = main_execute_part4('orchid.jpg', 3)
    colorImSave('orchid_part4_rad3_output.png', canvas_1_part4)
    
    canvas_2_part4 = main_execute_part4('RV.jpg', 3)
    colorImSave('RV_part4_rad3_output.png', canvas_2_part4)
    
    # call appropriate function and produce output for both images for Part 4,
    # for radius = 1
    canvas_1_part4 = main_execute_part4('orchid.jpg', 1)
    colorImSave('orchid_part4_rad1_output.png', canvas_1_part4)
    
    canvas_2_part4 = main_execute_part4('RV.jpg', 1)
    colorImSave('RV_part4_rad1_output.png', canvas_2_part4)
    
    # call appropriate function and produce output for both images for Part 5,
    # for radius = 3
    canvas_1_part5 = main_execute_part5('orchid.jpg', 3)
    colorImSave('orchid_part5_rad3_output.png', canvas_1_part5)
    
    canvas_2_part5 = main_execute_part5('RV.jpg', 3)
    colorImSave('RV_part5_rad3_output.png', canvas_2_part5)
    
    # call appropriate function and produce output for both images for Part 5,
    # for radius = 1
    canvas_1_part5 = main_execute_part5('orchid.jpg', 1)
    colorImSave('orchid_part5_rad1_output.png', canvas_1_part5)
    
    canvas_2_part5 = main_execute_part5('RV.jpg', 1)
    colorImSave('RV_part4_rad5_output.png', canvas_2_part5)
    
    # call appropriate function and produce output for both images for final
    # part, Part 6
